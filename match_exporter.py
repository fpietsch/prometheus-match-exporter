...
def find_text_in_file(path, match):
	if debug:
			print "Search in File %s for %s"%(path,match)
	file = open(path,"r+")
...
def update_matches():
	for name, state in states.items():
		states[name] = find_text_in_file(files[name],matches[name])
		if debug:
...
class CustomCollector(object):
    def collect(self):
        c = GaugeMetricFamily('match', 'Matches Found in Textfiles', labels=[
                                'name'])
        for name, state in states.items():
            if debug:
